...
var socket = io.connect('https://glacial-oasis-58475.herokuapp.com');

socket.on('connect', function() {
	console.log('Client connected');
});

socket.on('receive-thought', function(data) {
	//Recieved thought -- check if it is for us
	chrome.storage.sync.get([ 'username', 'partner_name', 'thought_count' ], function(result) {
		console.log('data received: ' + JSON.stringify(data));
		if (result.username == '' || result.partner_name == '') {
			chrome.runtime.sendMessage('', {
				type: 'error',
				options: {
					title: 'Error Found !',
					message: 'It seems your username/partner name is not filled out!',
					iconUrl: './error.png',
					type: 'basic'
				}
			});
			...
			if (result.partner_name== data.sender) {
				...
				chrome.runtime.sendMessage('', {
					...
					options: {
						...
						message: 'Your partner is thinking about you!',
						iconUrl: './heart.png',
						type: 'basic'
					}
				});

                //Increase thoughtcount
                if(result.thought_count == undefined){
                    chrome.storage.sync.set({ thought_count: 1 }, () => {
                        console.log('Thought Count in storage reset to: ' + 1);
                        document.getElementById('thought-count').innerHTML = 1;
                    });
                }else {
                    console.log(typeof(result.thought_count));
                    chrome.storage.sync.set({ thought_count: 1+result.thought_count }, () => {
                        console.log('Thought Count in storage set to: ' + 1+parseInt(result.thought_count));
                        document.getElementById('thought-count').innerHTML = 1+parseInt(result.thought_count);
                    });
                }
...
function confirmButtonOnClick() {
	...
	if (document.getElementById('username').value != '' && document.getElementById('partner-name').value != '') {
		...
		chrome.storage.sync.set({ thought_count: 0 }, () => {
			console.log('Thought Count in storage reset to: ' + 0);
		});
...
function sendThoughts() {
	//Get username and partner name from storage.sync

	chrome.storage.sync.get([ 'username', 'partner_name' ], function(result) {
		console.log('send results, getting username from storage ' + result);
		console.log('username found is ' + result.username);
		console.log('partner-name found is ' + result.partner_name);
		//If they are empty, send error notification
		if (result.username == '' || result.partner_name == '') {
			chrome.runtime.sendMessage('', {
				type: 'error',
				options: {
...
async function verifySetup() {
	await chrome.storage.sync.get([ 'username', 'partner_name' ], function(result) {
		if (result.username != '' && result.partner_name != '') {
